...
| link:https://vertx.io/docs/apidocs/io/vertx/ext/web/client/WebClientOptions.html[WebClientOptions]
| Set the WebClientOptions that will be used when creating the Vert.x WebClients.The default value sets:
* a connect timeout of 10s to avoid excessive delays on bad URLs
* a read idle timeout of 10s to avoid issues with upstream servers hanging
* an idle timeout of 10 to avoid issues with upstream servers hanging
...
| [[writeStreamBufferSize]]writeStreamBufferSize
| int
| Set the WriteStream buffer size.The amount of data to cache before writing to the output stream.
...
!===
! Buffer Size ! When to Use It ! Notes 

! 8KB
! Low-latency, chatty streams
! Matches TCP segment size on many systems

! 16KB
! Balanced throughput and responsiveness
! Good default for HTTP/1.1

! 32KB
! High-throughput, large payloads
! Reduces write calls, but increases latency

! 64KB
! Bulk transfer (e.g. file streaming)
! Only if memory pressure is low 


!===


| [[responseWriteQueueMaxSize]]responseWriteQueueMaxSize
| int
| Set the maximum number of buffers in the response queue before back-pressure kicks in.Without setting the WriteQueueMaxSize the Netty buffers can get very large with no back pressure.
...
| [[enableForceJwt]]enableForceJwt
| boolean
| Set the enableForceJwt parameter.If true, the path /login/forcejwt can be PUT to create a session based on the the JWT in the message body.
...
| [[operators]]operators
| uk.co.spudsoft.query.main.Operators

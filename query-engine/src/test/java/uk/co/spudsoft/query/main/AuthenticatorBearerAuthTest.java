...
public class AuthenticatorBearerAuthTest {

  private static final Logger logger = LoggerFactory.getLogger(AuthenticatorBearerAuthTest.class);
  ...
  public void testBuildRequestContext(Vertx vertx, VertxTestContext testContext) throws Exception {
    Authenticator rcb = new Authenticator(WebClient.create(vertx), validator, discoverer, new LoginDaoMemoryImpl(Duration.ZERO), null, null, true, null, true, null, Collections.singletonList("aud"), null);
            ...
            .handler(ctx -> {
              ...
              rcb.authenticate(ctx, requestContext)
                      ...
                      .onSuccess(req -> {
                        ...
                        HttpServerResponse response = ctx.response();
                        response.setStatusCode(200);
                        response.end("Hello " + req.getName());
                      });
            });

...
public class LogoutConfigMemoryBackIT extends AbstractLoginLogoutTester {

  private static final Logger logger = LoggerFactory.getLogger(LogoutConfigMemoryBackIT.class);
  ...
  /**
   * An extension to the usual tests to try calling login whilst already having a cookie.
   * @throws Exception 
   */
  @Test
  public void testLoginTwice() throws Exception {
    ...
    String location = given()
            ...
            .cookies(cookies)
            .log().all()
            .get("/login?provider=test&return=/")
            .then()
            .statusCode(307)
            .log().all()
            .extract().header("Location");
    assertThat(location, equalTo("http://localhost:" + main.getPort() + "/"));

    logger.debug("Cookies: {}", cookies);
    cookies.put("qe-session", "invalid");
    ...
    location = given()
            ...
            .log().all()
            .extract().header("Location");
    assertThat(location, startsWith("http://localhost:" + oidc.getActualPort() + "/auth?response_type=code"));

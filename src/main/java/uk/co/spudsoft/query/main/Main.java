...
private static final String MAVEN_PROJECT_VERSION = "0.0.1-10-docs-SNAPSHOT";
...
private static final String NAME = "query-engine";
  ...
  private Future<Integer> innerMain(String[] args) {   
    ...
    controllers.add(new InfoHandler(rcb, defnLoader, params.isDesignMode()));
    if (params.isDesignMode()) {
      controllers.add(new DesignHandler(vertx, defnLoader, dirCache));
    }
    ...
    router.route(QueryRouter.PATH_ROOT + "/*").handler(new QueryRouter(vertx, auditor, rcb, defnLoader, pipelineExecutor, params.isDesignMode()));
